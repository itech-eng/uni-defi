...
async function getUncollectedFees(
  ...
  // const poolInfo = await getPoolInfo(token0_obj, token1_obj, pool_fee, pool_factory_address);
  ...
  const fee_token0 = (results.amount0 / 10 ** Number(token0_obj.decimals)).toFixed(Number(token0_obj.decimals));
  const fee_token1 = (results.amount1 / 10 ** Number(token1_obj.decimals)).toFixed(Number(token1_obj.decimals));

  console.log("fee_token0: ", fee_token0.toString());
  console.log("fee_token1: ", fee_token1.toString());
}

async function getPoolInfo(token0_obj, token1_obj, pool_fee, pool_factory_address) {
  if (!provider) {
